...
abstract contract ERC1155 is Context, ERC165, IERC1155, IERC1155MetadataURI, IERC1155Errors {
     ...
     */
    constructor() {
        _setURI("https://farfantasy.com/api/moxiecards/metadata/{id}");
    }
    ...
    function _doSafeTransferAcceptanceCheck(
        ...
        if (to.code.length > 0) {
            ...
            //Only whitelisted contracts can receive / Sent NFTs
            if(whitelistedContracts[to] == false){
...
contract FarfantasyMoxieCards is ERC1155, Ownable, ERC1155Supply {
    ...
    //Locked Fan Tokens for each user
    mapping(address ft => mapping(address user => uint256 amount)) public lockedFT;
    ...
    function mint(address subject_ft, uint256 cards) public {
        ...
        //Must mint atleast 1 card
        require(cards > 0, "Invalid Cards");

        //Get the total supply of the FT ERC20
        uint256 totalSupply = IFanTokensERC20(subject_ft).totalSupply();

        //FT Amount should be more than 0
        uint256 amount_ft = (totalSupply * cards) / AMOUNT_RATIO_TO_SUPPLY;
        require(amount_ft > 0, "Insufficient Fees");

        //Transfer the FT from the user to this contract
        IFanTokensERC20(subject_ft).transferFrom(msg.sender, address(this), amount_ft);
        ...
        minted[subject_ft][msg.sender] += cards;

        //Mint the cards
        _mint(msg.sender, fid, cards, "");
        
        //Announce the mint
        emit MoxieCard(msg.sender, subject_ft, fid, cards, amount_ft, true, block.timestamp);
    }
    ...
    function mintBatch(address[] calldata subject_fts, uint256[] calldata cards) public {
        ...
        for (uint256 i = 0; i < subject_fts.length; i++) {
            ...
            //Transfer the FT from the user to this contract
            IFanTokensERC20(subject_fts[i]).transferFrom(msg.sender, address(this), amount_ft);
    ...
    function burn(address subject_ft, uint256 cards) public {       
        ...
        //Minimum 1 card should be burned
        require(cards > 0, "Invalid Cards");

        //Number of card balance should more or equal then the cards to withdrawable
        uint256 _cardBalance = balanceOf(msg.sender, fid);
        require(_cardBalance >= cards, "Insufficient cards balance");

        //Get the minted number of cards by this user
        uint256 _cardMinted = minted[subject_ft][msg.sender];
        require(_cardMinted >= cards, "Mismatch Mints");

        //Get the locked FT amount of this User
        uint256 amount_ft = lockedFT[subject_ft][msg.sender];
        
        //Calculate the withdrawable amount
        uint256 withdrawAmount = ((amount_ft * cards) / _cardMinted);

        //Minimum amount of FT that locked should be more or equal of Withdrawable amount.
        require(amount_ft >= withdrawAmount, "Insufficient FT balance");
        ...
        IFanTokensERC20(subject_ft).transfer(msg.sender, withdrawAmount);

        //Burn the cards
        _burn(msg.sender, fid, cards);

        //Announce the burn
        emit MoxieCard(msg.sender, subject_ft, fid, cards, withdrawAmount, false, block.timestamp);
    }
    ...
    function burnBatch(address[] calldata subject_fts, uint256[] calldata cards) public {
        ...
        for (uint256 i = 0; i < fids.length; i++) {
            ...
            //Cards should be more than 0
            require(cards[i] > 0, "Invalid Cards");

            //Number of card balance should more or equal then the cards to withdrawable
            _cardBalance = balanceOf(msg.sender, fid);
            ...
            //Transfer the FT to the user
            IFanTokensERC20(subject_fts[i]).transfer(msg.sender, withdrawAmount);

            emit MoxieCard(msg.sender, subject_fts[i], fid, cards[i], withdrawAmount, false, block.timestamp);

        }

        _burnBatch(msg.sender, fids, cards);
    }
    ...
    //Map the FT ERC20 to FID
    event MappedFT2FID(address indexed FT_ERC20, uint256 indexed fid, uint256 timestamp);
    function updateSubjectERC20FID(address _subjectERC20, uint256 fid) public onlyOwner {
        //FID must be a valid FID
        require(fid > 0, "Invalid FID");

        //FT address must be a valid address
        require(_subjectERC20 != address(0), "Invalid Subject FT");

        //Map the FT ERC20 to FID
        subjectERC20FID[_subjectERC20] = fid;

        //Announce the mapping
        emit MappedFT2FID(_subjectERC20, fid, block.timestamp);
    }

    function updateSubjectERC20FIDBulk(address[] calldata _subjectERC20s, uint256[] calldata fids) public onlyOwner {
        //Parameters should be equal
        require(_subjectERC20s.length == fids.length && _subjectERC20s.length > 0, "Mismatch");

        for (uint256 i = 0; i < _subjectERC20s.length; i++) {
            //FID must be a valid FID
            require(fids[i] > 0, "Invalid FID");

            //FT address must be a valid address
            require(_subjectERC20s[i] != address(0), "Invalid Subject FT");

            //Map the FT ERC20 to FID
            subjectERC20FID[_subjectERC20s[i]] = fids[i];

            //Announce the mapping
            emit MappedFT2FID(_subjectERC20s[i], fids[i], block.timestamp);
        }
    }

    function whitelistContracts(address _contract, bool _status) public onlyOwner {
    ...
    function isUniqueArray(uint256[] calldata _keys) internal pure returns (bool) {
    ...
    //Check if array items are unique for address arrays 
    function isUniqueArray(address[] calldata _keys) internal pure returns (bool) {
        for(uint256 i = 0; i < _keys.length; i++) {
            for(uint256 j = i + 1; j < _keys.length; j++) {
                if(_keys[i] == _keys[j]) {
                    return false;
